...
export class LikeAlbumComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.likeAlbumService
      ...
      .subscribe(
        (res: HttpResponse<ILikeAlbum[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  registerChangeInLikeAlbums(): void {
    this.eventSubscriber = this.eventManager.subscribe('likeAlbumListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: ILikeAlbum[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/like-album'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
