...
export class ExtendedUserComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.extendedUserService
      ...
      .subscribe(
        (res: HttpResponse<IExtendedUser[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  trackId(index: number, item: IExtendedUser): number {
  ...
  delete(extendedUser: IExtendedUser): void {
    const modalRef = this.modalService.open(ExtendedUserDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
  ...
  protected onSuccess(data: IExtendedUser[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/extended-user'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
